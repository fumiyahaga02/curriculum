...
//	 * @throws MVCException
	...
	protected boolean validateSession(HttpServletRequest request, HttpServletResponse response)
			throws IOException, MVCException {
		Logger.logStart(new Throwable());

		boolean hasSession = false;

		// セッションを取得
//        getSessionメソッドはサーブレットを要求してきたクライアントに対してセッションがすでに開始されていればそのセッションを返す。
//        引数にtureを指定した場合セッションが開始されていなければ,新規にセッションを開始したあとでそのセッションを返す。
//        引数にfalseを指定した場合セッションが存在しない場合にnullが返ってくる
		HttpSession session = request.getSession(false);

//        isNullメソッド 指定された参照がnullの場合はtrueを返します。それ以外の場合はfalseを返します。
//        isNewメソッドは、取得したセッションが新規に生成したセッションかどうか判定します。
//        新規に生成したセッションならtrueを、既存のセッションを取得した場合は、falseを返します。
...
//	 * @throws IOException
	...
	protected void setRedirectInfo(final boolean isLogout, HttpSession session, HttpServletRequest request,
		...
		if (!isLogout)
			session.setAttribute(CONST_REQUST_KEY_FOR_REDIRECT, ConstMessage.ERROR_SESSION_INVALID);
